...
export default function StudentDashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-[#fdf8f5] md:p-6 lg:p-8">
      {/* Container simulating the white dashboard background */}
      <div className="flex-1 flex bg-white md:rounded-[2.5rem] md:shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden border border-orange-50/50">
        <Sidebar userType="student" />

        <main className="flex-1 overflow-y-auto">
          {/* Header */}
          <header className="px-6 md:px-10 py-6 md:py-8 flex justify-between items-center bg-white sticky top-0 z-20">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-slate-800">
                Welcome back, {user?.firstName}
              </h1>
              <p className="text-sm text-slate-500 font-medium mt-1">Nice to see you again!</p>
            </div>
          ...
          <div className="p-6 md:p-10 pt-0 max-w-[1600px] mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <section>
                  <div className="flex justify-between items-center mb-5">
                    ...
                    <button className="flex items-center gap-1 px-3 py-1.5 bg-red-400 text-white rounded-full text-xs font-bold hover:bg-red-500 transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <section className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] p-6">
                    <div className="flex justify-between items-center mb-6">
                      <h3 className="font-bold text-slate-800">Your Level</h3>
                    </div>
                  ...
                  <section className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] p-6">
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">Jump right in</p>
                      </div>
                      <select className="text-xs font-medium text-slate-500 bg-transparent border-none outline-none cursor-pointer">
                        <option>Today</option>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {quickActions.map((action, i) => (
                        <button
                          ...
                          className="flex flex-col items-center justify-center gap-3 p-4 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors group"
                        >
                          ...
                          <span className="font-semibold text-xs text-slate-700">{action.label}</span>
                        </button>
                      ))}
                    </div>
                  </section>
                ...
                <section>
                  <div className="flex justify-between items-center mb-5">
                    <h2 className="text-lg font-bold text-slate-800">Course Leaderboard</h2>
                    <button className="flex items-center gap-1 px-3 py-1.5 bg-red-400 text-white rounded-full text-xs font-bold hover:bg-red-500 transition-colors">
                      ...
                      <table className="w-full text-left text-sm">
                        ...
                        <tbody>
                          {[
                            ...
                            <tr key={i} className={`border-b last:border-0 border-slate-50 transition-colors hover:bg-slate-50/50 ${row.active ? 'bg-orange-50/30' : ''}`}>
                              <td className="py-4 px-6">
                                <div className="flex items-center gap-3">
                                  <span className={`font-bold ${row.active ? 'text-slate-800' : 'text-slate-500'}`}>{row.rank}</span>
                                  {row.active ? (
                              ...
                              <td className="py-4 px-6 text-slate-600 font-medium">{row.course}</td>
                              <td className="py-4 px-6 text-right font-bold text-slate-800">{row.score}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="space-y-8">
                ...
                <section>
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-md transition-all cursor-pointer group">
                        <div className={`w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center ${session.color} group-hover:bg-slate-100 transition-colors`}>
                          <session.icon className="w-6 h-6" />
                        </div>
                        <div>
                          <h4 className="font-bold text-sm text-slate-800 group-hover:text-orange-500 transition-colors">{session.title}</h4>
                ...
                <section>
                  <div className="flex justify-between items-center mb-5">
                    <h2 className="text-lg font-bold text-slate-800">Tutors</h2>
                    <button className="text-slate-400 hover:text-slate-600 transition-colors">
                      <MoreHorizontal className="w-5 h-5" />
                    </button>
                  </div>
                    ...
                    <div className="space-y-6">
                      {[
                        ...
                        <div key={i} className="flex items-center justify-between group cursor-pointer">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-slate-100 text-slate-600 font-bold flex items-center justify-center group-hover:bg-orange-100 group-hover:text-orange-600 transition-colors">
                              {friend.img}
                            </div>
                            <div>
                              <h4 className="font-bold text-sm text-slate-800 group-hover:text-orange-500 transition-colors">{friend.name}</h4>
                              <p className="text-xs text-slate-500 mt-0.5">{friend.role}</p>
                            </div>
                          </div>
                          <button className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 group-hover:bg-orange-50 group-hover:text-orange-500 group-hover:border-orange-200 transition-all">
